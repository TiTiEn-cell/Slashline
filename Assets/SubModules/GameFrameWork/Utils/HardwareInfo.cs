...
namespace GFramework.Utils
{
    ...
    public class HardwareInfo : SingletonMono<HardwareInfo> {
		const string fpsPenaltyKey = "fpsPenalty";
		float fpsPenalty;
		public float FpsPenalty
		{
			get => fpsPenalty;
			set
			{
				fpsPenalty = Mathf.Clamp01(value);
				PlayerPrefs.SetFloat(fpsPenaltyKey, fpsPenalty);
				PlayerPrefs.Save();
			}
		}
		...
		void Start (){
			fpsPenalty = PlayerPrefs.GetFloat(fpsPenaltyKey, 0);

			referenceConfiguration.initialized = true;
			comparisonWeights.initialized = true;
			...
			userHardware += userConfiguration.deviceType + "\n";
			userHardware += userConfiguration.gpuDeviceType + "\n";
			userHardware += userConfiguration.gpuMemory + "\n";
			userHardware += userConfiguration.gpuMultiThread + "\n";
			userHardware += userConfiguration.gpuShaderLevel + "\n";
			userHardware += userConfiguration.maxTextureSize + "\n";
			userHardware += userConfiguration.processorCount + "\n";
			userHardware += userConfiguration.processorFrequency + "\n";
			userHardware += userConfiguration.supportsComputeShaders + "\n";
			userHardware += userConfiguration.supportsShadows + "\n";
			userHardware += userConfiguration.systemMemory + "\n";
		...
		private void OnGUI()
		{
			...
			GUILayout.Label($"\n\nFPS: {(Time.frameCount / Time.time)}\n\nHSCORE: {Mathf.RoundToInt(userHardwareScore)} - {DeviceProfile.GetDeviceProfile()} \n\n {compatibilityWarnings} \n\n {userHardware}");
		}
	}
