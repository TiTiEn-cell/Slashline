...
#pragma warning disable
        ...
        public static void CreateDirectory(string folder, bool replace = false)
        {
        ...
        public static void SaveFileToResource(string data, string path)
        {
            //write file to storage
            var resourcePath = Application.dataPath + "/Resources/" + path;
            File.WriteAllText(resourcePath, data);
        }
        ...
        /// <param name="filePath">Path to the file</param>
        /// <param name="isAbsolutePath">Is this path an absolute one?</param>
        /// <returns>Data of the file, in string[] format</returns>
        public static string[] LoadTextFile(string filePath, bool isAbsolutePath = false)
        {
            if (filePath == null || filePath.Length == 0)
            {
                return null;
        ...
        /// Save a byte array to storage at specified path and return the absolute path of the saved file
        /// </summary>
        /// <param name="strings">Data to write</param>
        /// <param name="filePath">Where to save file</param>
        /// <param name="isAbsolutePath">Is this path an absolute one or relative</param>
        /// <returns>Absolute path of the file</returns>
        public static string SaveTextFile(string strings, string filePath, bool isAbsolutePath = false, bool isSaveResource = false)
        {
            if (filePath == null || filePath.Length == 0)
            {
                return null;
            ...
            if (isSaveResource) {
                SaveFileToResource(strings, filePath);
            }
